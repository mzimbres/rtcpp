...
To address these problems, we propose that node-based containers favor the
overload \texttt{allocate()}, that can serve only one-size memory blocks, over
\texttt{allocate(size\_type)}, whenever \texttt{std::allocator\_traits}
instructs them so.  The same reasoning applies to \texttt{deallocate()}.
...
struct allocator_traits {
  ...
  using use_node_allocation = typename Alloc::use_node_allocation;
};
\end{lstlisting}
