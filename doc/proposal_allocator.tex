...
the allocator member function \texttt{allocate()}, that can serve only one-size
memory blocks, over \texttt{allocate(size\_type)}, whenever this overload is
available.  The same reasoning is applies to \texttt{deallocate()}.
...
\item Containers in major libraries always use $n = 1$, which
