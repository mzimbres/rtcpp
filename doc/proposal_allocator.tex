...
is not being met on node-based containers due to the current suboptimal
allocator interface. This proposal tries to fix what the author
...
two allocation schemes by means of a non-breaking addition to
...
Many linked lists served with node stored in an \texttt{std::array.}
