...
The performance gain compared to the standard allocator in remarkable.
...
use $n = 1$, they may simply be implemented with
\texttt{allocate\_node()}, regardless of whether the allocator provides this
function or not. The implementation of \texttt{allocate\_node()} 
falls back to \texttt{allocate(1)} when the allocator does not provide one.
